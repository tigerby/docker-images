...
export CONNECT_KEY_CONVERTER_SCHEMAS_ENABLE=$KEY_CONVERTER_SCHEMAS_ENABLE
export CONNECT_VALUE_CONVERTER_SCHEMAS_ENABLE=$VALUE_CONVERTER_SCHEMAS_ENABLE
...
export CONNECT_CONNECTOR_CLIENT_CONFIG_OVERRIDE_POLICY=$CONNECTOR_CLIENT_CONFIG_OVERRIDE_POLICY
if [[ -n "$HEAP_OPTS" ]]; then
...
if [ "$ENABLE_JMX_EXPORTER" = "true" ]; then
  KAFKA_OPTS="${KAFKA_OPTS} -javaagent:$(ls "$KAFKA_HOME"/etc/jmx_prometheus_javaagent*.jar)=9404:$KAFKA_HOME/etc/jmx_exporter_config.yaml"
  export KAFKA_OPTS
fi
...
case $1 in
    start)
        ...
        echo "      CONNECTOR_CLIENT_CONFIG_OVERRIDE_POLICY=$CONNECT_CONNECTOR_CLIENT_CONFIG_OVERRIDE_POLICY"
